#!/usr/bin/env python3
"""Daily memory optimization — L1/L2/L3 maintenance for Hermes Agent (v3.4).

no-agent cron script: stdout is delivered verbatim; empty stdout = silent.

v3.4 (Sep 2026): Expanded remediation allowlist — all issue codes now have
a remediation action. Post-action info issues (smoke-test cleanup,
meta-memory cleanup, possible-duplicate report, L3 lint report) are marked
resolved (work already done). KP staleness triggers consolidation. USER.md
near-capacity triggers transactional offload to L2. No more unresolved
"informational" issues in Telegram notifications.
  - mark_resolved action: acknowledges post-action info issues
  - prune_user_md action: transactional USER.md offload (classify → retain → remove)
  - KP_PAGES_STALE: triggers consolidation (may refresh projected pages)
  - L3_LINT_REPORT: marked resolved (lint already ran, report is informational)
...
RULE_REMEDIATIONS = {
    ...
    # v3.4: post-action info issues — work already done, just acknowledge
    "SMOKE_TEST_CLEANUP": "mark_resolved",
    "META_MEMORY_CLEANUP": "mark_resolved",
    "POSSIBLE_DUPLICATE_REPORT": "mark_resolved",
    "L3_LINT_REPORT": "mark_resolved",
    # v3.4: KP staleness — trigger consolidation (may refresh projected pages)
    "KP_PAGES_STALE": "trigger_consolidation",
    # v3.4: USER.md near capacity — attempt offload of offloadable entries
    "L1_USER_NEAR_CAPACITY": "prune_user_md",
}
...
def try_resolve_issues_with_rules(issues):
    ...
    - Resolve stable conflicts (CONFLICT_FLAG_HUMAN)
    - Execute actions based on generated natural-language instructions

    v3.4: The allowlist now covers all issue codes generated by this script.
    Post-action info issues use mark_resolved (acknowledge only). KP staleness
    triggers consolidation. USER.md near-capacity triggers transactional offload.
    Only genuine error conditions (check failures, timeouts, critical smoke-test
    failures) and human-review contradictions remain unresolved.
...
def _prune_user_md(issue):
    """v3.4: Prune USER.md by offloading non-essential entries to Hindsight.

    Uses a conservative approach for USER.md (user profile facts):
    - Only offload entries that match hard-offload patterns (completed tasks,
      old state, maintenance reports) OR are confirmed duplicates in Hindsight.
    - Durable preferences, conventions, and identity facts are ALWAYS kept —
      they don't match the MEMORY.md essential prefixes but are still essential.
    - Transactional: only removes entries confirmed in L2.

    Returns resolution text on success, None if no progress was made.
    """
    ...
    try:
        ...
        # Conservative classification for USER.md:
        # Only offload entries matching hard-offload patterns (completed tasks,
        # old state, expired incidents). Durable preferences/conventions are kept.
        offloadable = []
        essential = []
        for entry in entries:
            is_offloadable = False
            # Check against hard-offload regex patterns
            for pattern in memory_heuristics.DEFAULT_OFFLOAD_PATTERNS:
                if re.search(pattern, entry, re.IGNORECASE):
                    is_offloadable = True
                    break
            if is_offloadable:
                offloadable.append(entry)
            else:
                essential.append(entry)

        if not offloadable:
            # All entries are durable preferences — can't auto-prune
            issue.message += " — all entries are durable preferences, manual prune needed"
            return None

        # Transactional offload: retain to L2, track safety
        entries_to_keep = list(essential)
        safe_to_remove = []
        for entry in offloadable:
            tags = ["user-profile", "offloaded"]
            # Dedup check against Hindsight
            if memory_offload and hasattr(memory_offload, "hindsight_recall_check"):
                if memory_offload.hindsight_recall_check(entry):
                    safe_to_remove.append(entry)
                    continue
            # Retain to L2
            if memory_offload and hasattr(memory_offload, "hindsight_retain"):
                if memory_offload.hindsight_retain(entry, tags):
                    safe_to_remove.append(entry)
                else:
                    entries_to_keep.append(entry)  # keep on failure
            else:
                entries_to_keep.append(entry)  # no offload module — keep

        if not safe_to_remove:
            issue.message += " — offload failed for all candidates, manual prune needed"
            return None

        # Rewrite USER.md atomically (essential + failed entries only)
        new_content = "".join(f"{entry.strip()}\n§\n" for entry in entries_to_keep)
        _atomic_write_text(USER_FILE, new_content)

        new_len = len(new_content)
        old_len = len(content)
        memory_heuristics.audit_log(
            operation="user_md_prune",
            memory_id="USER.md",
            rule="L1_USER_NEAR_CAPACITY",
            confidence="high",
            reason=f"offloaded {len(safe_to_remove)} entries to L2, "
                   f"{old_len}→{new_len} chars",
        )
        return (f"{render_issue(issue)} → resolved "
                f"(offloaded {len(safe_to_remove)} entries, "
                f"now {new_len}/{USER_CAP} chars)")
    except Exception:
        return None
...
def _execute_rule_remediation(issue: Issue, action: str):
    ...
    try:
        ...
        if action == "mark_resolved":
            # v3.4: post-action info issues (smoke-test cleanup, meta-memory
            # cleanup, possible-duplicate report, L3 lint report) — the work
            # was already done before the issue was created. Just acknowledge.
            return f"{render_issue(issue)} → resolved (informational, no action needed)"

        if action == "prune_user_md":
            # v3.4: USER.md near capacity — attempt to offload entries to L2.
            # Uses the same transactional pattern as memory_offload: classify,
            # retain to Hindsight, then remove from USER.md. Only removes
            # entries confirmed in L2.
            if not ALLOW_DESTRUCTIVE:
                return None
            return _prune_user_md(issue)
        ...
        if USER_FILE.exists():
            ...
            if pct >= MEM_WARN:
                issues.append(Issue(
                    ...
                    context={"file": str(USER_FILE), "chars": n, "cap": USER_CAP},
                    auto_resolvable=True,
                ))
    except Exception as e:
        issues.append(Issue(
